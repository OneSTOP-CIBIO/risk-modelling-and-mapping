...
# --- Parallel intersection setup -------------------------------------------
# terra objects are external pointers to C++ memory and cannot be sent to a
# worker process as-is. They must be wrapped (serialized) before transport and
# unwrapped (rebuilt) inside the worker. The two boundary vectors are constant
# across the loop, so pack them a single time here.
# NOTE: this is Windows (see the D:/ paths), so fork-based sharing (multicore)
# is unavailable; multisession spawns separate R processes, which is exactly
# why the wrap/unwrap round-trip is required.
eu_bounds_pk <- terra::wrap(eu_bounds_vec)
eu_bbox_pk   <- terra::wrap(eu_bbox_vec)

# Set the plan ONCE, outside the loop. Doing it inside would respawn the
# worker processes on every species iteration.
...
for (i in seq_along(sp_data_files)) {
  ...
  ##
  ## Each of the four intersections is dispatched to a separate worker and the
  ## results are collected back in input order, so the four objects below are
  ## identical to what sequential terra::intersect() calls would produce.
  ##
  ...
  # Pack the per-species point vectors for transport to the workers. The live
  # sp_data_global_vect is still used later (terra::extract), so it is left
  # untouched -- only a packed copy is sent out.
  sp_points_pk     <- terra::wrap(sp_data_global_vect)
  raw_sp_points_pk <- terra::wrap(raw_sp_data_global_vect)
  
  intersect_jobs <- list(
    country     = list(points = sp_points_pk,     target = eu_bounds_pk),
    bbox        = list(points = sp_points_pk,     target = eu_bbox_pk),
    raw_country = list(points = raw_sp_points_pk, target = eu_bounds_pk),
    raw_bbox    = list(points = raw_sp_points_pk, target = eu_bbox_pk)
  )
  
  intersect_results <- future_lapply(
    intersect_jobs,
    function(job) {
      pts <- terra::unwrap(job$points)
      tgt <- terra::unwrap(job$target)
      terra::wrap(terra::intersect(pts, tgt))
    },
    future.packages = "terra",  # ensure terra is attached on each worker
    future.seed = TRUE          # silence the parallel-RNG check
  )
  
  # future_lapply preserves input order, so this mapping is exact. Unwrap each
  # result back into a live SpatVector for the rest of the workflow.
  country_intersections     <- terra::unwrap(intersect_results$country)
  bbox_intersections        <- terra::unwrap(intersect_results$bbox)
  raw_country_intersections <- terra::unwrap(intersect_results$raw_country)
  raw_bbox_intersections    <- terra::unwrap(intersect_results$raw_bbox)
