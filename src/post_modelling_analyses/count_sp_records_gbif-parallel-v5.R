...
file_path_eur <- paths$file_path[10]
r_eur <- terra::rast(file_path_eur)

raster_path        = file_path
raster_eur_path    = file_path_eur
...
process_species <- function(sp_data_path,
  ...
  if (is.null(cache)) {
    ...
    r_eur <- terra::rast(raster_eur_path) 
    # lazy: header only
    eu_bounds_vec <- terra::vect(bounds_path)
    if (!terra::same.crs(eu_bounds_vec, r)) {
      eu_bounds_vec <- terra::project(eu_bounds_vec, terra::crs(r))
  ...
  writeVector(sp_data_global_vect, global_filt_file_path)
  ...
  writeVector(raw_sp_data_global_vect, raw_file_path)
  ...
  extracted_eur_cells <- terra::extract(
    r_eur,
    sp_data_global_vect,
    cells = TRUE,
    ID = TRUE
  ...
  n_unique_eur_bbox <- n_distinct(extracted_eur_cells$cell)
  ...
  tmp_counts <- data.frame(
    ...
    n_unique_eur_bbox = n_unique_eur_bbox,
