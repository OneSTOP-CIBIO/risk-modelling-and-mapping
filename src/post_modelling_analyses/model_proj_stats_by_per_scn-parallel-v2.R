# ==========================================================================
# Zonal map-statistics sweep  ---  parallel, instrumented
#
# Supports binary suitable-cell cover and continuous favourability summaries.
...
source("./src/post_modelling_analyses/model_proj_stats_helpers.R")
...
CFG <- list(
  calculation_mode = "binary_maps",#"favourability_maps", 
  ...
  #max_tasks     = 140     # e.g. 140 for a smoke test; needs >= ~2x workers groups
  
)

MODE_CFG <- model_proj_mode_config(
  CFG$calculation_mode,
  output_dir = CFG$output_dir
)
CFG$path_list_field <- MODE_CFG$path_list_field
CFG$map_used <- MODE_CFG$map_used
CFG$out_file <- MODE_CFG$out_file
CFG$out_csv_file <- MODE_CFG$out_csv_file
CFG$region_out_file <- MODE_CFG$region_out_file
CFG$region_out_csv_file <- MODE_CFG$region_out_csv_file
CFG$err_file <- MODE_CFG$err_file
CFG$perf_file <- MODE_CFG$perf_file
CFG$region_name <- tools::file_path_sans_ext(basename(CFG$bounds_gpkg))
...
cli::cli_h1("Setup")
cli::cli_alert_info(
  "Calculation mode: {.val {CFG$calculation_mode}} using {.field {CFG$path_list_field}}"
)
...
zone_lookup <- tasks |>
  ...
  mutate(
    geom_hash = substr(
      vapply(geom, rlang::hash, character(1L)),
      1L,
      10L
    )
  ) |>
  transmute(
    ...
    zone_file = file.path(
      CFG$zone_dir,
      sprintf("zones_%s.tif", geom_hash)
    ),
    region_mask_file = file.path(
      CFG$zone_dir,
      sprintf("region_mask_%s.tif", geom_hash)
    )
  )
...
phase("rasterize_zones", {
  ...
  if (length(todo)) {
    pb <- cli::cli_progress_bar(
      name = "Rasterizing country zones",
      total = length(todo),
      clear = FALSE
    )
    for (j in seq_along(todo)) {
      ...
      cli::cli_progress_update(
        id = pb,
        set = j - 1L,
        status = basename(zone_lookup$zone_file[[i]])
      )
      ...
      cli::cli_progress_update(id = pb, set = j)
    }
    cli::cli_progress_done(id = pb)
  } else cli::cli_alert_success("Zone grids cached.")
})

phase("build_region_masks", {
  todo <- which(!file.exists(zone_lookup$region_mask_file))
  if (length(todo)) {
    pb <- cli::cli_progress_bar(
      name = "Building study-area masks",
      total = length(todo),
      clear = FALSE
    )
    for (j in seq_along(todo)) {
      ...
      cli::cli_progress_update(
        id = pb,
        set = j - 1L,
        status = basename(zone_lookup$region_mask_file[[i]])
      )
      region_mask <- model_proj_make_region_mask(
        rast(zone_lookup$zone_file[[i]])
      )
      writeRaster(
        region_mask,
        filename = zone_lookup$region_mask_file[[i]],
        overwrite = TRUE,
        wopt = list(
          datatype = "INT1U",
          gdal = c("COMPRESS=DEFLATE", "TILED=YES", "PREDICTOR=2")
    ...
    cli::cli_alert_success("Region masks cached.")
  }
})

tasks <- left_join(
  tasks,
  select(zone_lookup, geom, zone_file, region_mask_file),
  by = "geom"
)

# ---- 4. zonal statistics kernel (runs in workers) ------------------------

summarise_group <- function(df, n_zone, mem_cell_limit, n_workers, group_id,
                            calculation_mode, region_name,
                            exec = "parallel", pgr = NULL) {
  
  # A worker does not own the machine. nbrOfWorkers() returns 1 inside a
  # worker, so the worker count is passed in. tempdir must exist before it is set.
  ...
  region_mask <- terra::rast(df$region_mask_file[[1L]])
  zr <- terra::crop(
    terra::rast(df$zone_file[[1L]]),
    region_mask,
    snap = "near"
  )
  use_mem <- terra::ncell(region_mask) <= mem_cell_limit
  ...
  metric_columns <- function(result, n_rows, good) {
    if (identical(calculation_mode, "binary_maps")) {
      return(data.frame(
        n0 = if (good) result$n0 else rep(NA_integer_, n_rows),
        n1 = if (good) result$n1 else rep(NA_integer_, n_rows),
        total = if (good) result$total else rep(NA_integer_, n_rows)
      ))
    }

    data.frame(
      total = if (good) result$total else rep(NA_integer_, n_rows),
      mean_favourability = if (good) {
        result$mean_favourability
      } else {
        rep(NA_real_, n_rows)
      },
      sd_favourability = if (good) {
        result$sd_favourability
      } else {
        rep(NA_real_, n_rows)
      },
      median_favourability = if (good) {
        result$median_favourability
      } else {
        rep(NA_real_, n_rows)
      },
      mad_favourability = if (good) {
        result$mad_favourability
      } else {
        rep(NA_real_, n_rows)
      }
    )
  ...
  rows <- lapply(seq_len(nrow(df)), function(i) {
    ...
    if (!is.null(pgr)) {
      pgr(
        amount = 0,
        message = paste("Starting", progress_message)
      )
    }
    ...
    statistics <- tryCatch({
      ...
      xr <- model_proj_crop_mask(xr, region_mask)
      if (!is.null(pgr)) {
        pgr(
          amount = 0,
          message = paste("Masked", progress_message)
        )
      }
      ...
      if (use_mem) {
        ...
        if (!is.null(pgr)) {
          pgr(
            amount = 0,
            message = paste("Country statistics", progress_message)
          )
        }
        country_result <- model_proj_summarise_values(
          values = x,
          zones = zz,
          n_zone = n_zone,
        ...
        if (!is.null(pgr)) {
          pgr(
            amount = 0,
            message = paste("Region statistics", progress_message)
          )
        }
        region_result <- model_proj_summarise_values(
          values = x,
          zones = rep.int(1L, length(x)),
          n_zone = 1L,
          calculation_mode = calculation_mode
        )
        rm(x)
        list(country = country_result, region = region_result)
      } else {
        ...
        country_result <- model_proj_summarise_zonal(
          value_raster = xr,
          zone_raster = zr,
          n_zone = n_zone,
          calculation_mode = calculation_mode
        ...
        region_result <- model_proj_summarise_zonal(
          value_raster = xr,
          zone_raster = region_mask,
          n_zone = 1L,
          calculation_mode = calculation_mode
        )
        list(country = country_result, region = region_result)
    ...
    t1 <- as.numeric(Sys.time()); good <- is.list(statistics)
    if (!is.null(pgr)) {
      pgr(
        message = paste(
          if (good) "Finished" else "Failed",
          progress_message
        )
      )
    }

    country_common_columns <- data.frame(
      sp_name = row$sp_name,
      tkey = row$tkey,
      period = row$period,
    ...
    region_common_columns <- data.frame(
      sp_name = row$sp_name,
      tkey = row$tkey,
      period = row$period,
      scenario = row$scenario,
    ...
    list(
      country = cbind(
        country_common_columns,
        metric_columns(
          if (good) statistics$country else NULL,
          n_zone,
          good
        ),
        error = if (good) NA_character_ else statistics
      ),
      region = cbind(
        region_common_columns,
        metric_columns(
          if (good) statistics$region else NULL,
          1L,
          good
        ),
        error = if (good) NA_character_ else statistics
  ...
  list(country = do.call(rbind, lapply(rows, `[[`, "country")),
       region = do.call(rbind, lapply(rows, `[[`, "region")),
...
if (length(par_i)) {
  ...
  cli::cli_alert_info(
    "Starting parallel raster sweep: {n_par_rasters} raster{?s} across {length(par_i)} group{?s}."
  )
  flush.console()
    ...
    res_par <- with_progress(
      {
        pgr <- progressor(
          steps = n_par_rasters,
          label = "Parallel raster sweep"
        )
        future_map(
          par_i,
          function(i) summarise_group(
            groups[[i]],
            n_zone,
            CFG$mem_cell_limit,
...
assemble_statistics <- function(result_field) {
  assembled <- map(res, result_field) |>
    list_rbind() |>
    mutate(
      calculation_mode = CFG$calculation_mode,
      map_used = CFG$map_used
    )

  if (identical(CFG$calculation_mode, "binary_maps")) {
    assembled <- assembled |>
      mutate(
        pct_suitb = if_else(
          total > 0L,
          round(100 * n1 / total, 3),
          NA_real_
        )
      )
  }
...
suitability <- phase("assemble_country", {
  assemble_statistics("country") |>
...
region_suitability <- phase("assemble_region", {
  assemble_statistics("region") |>
    relocate(
      sp_name,
      tkey,
      calculation_mode,
...
if (identical(CFG$calculation_mode, "binary_maps")) {
  ...
  if (nrow(odd)) {
    cli::cli_alert_warning(
      "{nrow(odd)} row{?s} where n0+n1 != total: rasters hold values other than 0/1."
    )
  }
}
...
readr::write_csv(suitability, CFG$out_csv_file)
cli::cli_alert_success("Wrote {.file {CFG$out_file}} ({nrow(suitability)} rows)")
saveRDS(region_suitability, CFG$region_out_file)
readr::write_csv(region_suitability, CFG$region_out_csv_file)
cli::cli_alert_success(
  "Wrote {.file {CFG$region_out_file}} ({nrow(region_suitability)} rows)"
)
...
report_performance <- function(timings, gtimes, phases, workers,
  ...
  if (nrow(tp)) {
    ...
    work_profile <- c(
      ...
      "Computation"       = "{fmt_s(ct)} ({pct_of(ct, rd + ct)})",
    ...
    names(work_profile)[[2L]] <- if (
      identical(calculation_mode, "binary_maps")
    ) {
      "Counting"
    } else {
      "Favourability statistics"
    }
    cli::cli_dl(work_profile)
